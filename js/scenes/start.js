class Start extends Phaser.Scene {
    ...
    preload() {
        this.load.spritesheet("start-character", "assets/personagem/standard/walk.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
    }
    ...
    createButton(x, y, label, width = 180, height = 42) {
        const background = this.add.rectangle(0, 0, width, height, 0x062b16, 0.88)
    ...
    createColumns() {
        ...
        for (let index = 0; index < columnCount; index += 1) {
            ...
            for (let glyphIndex = 0; glyphIndex < length; glyphIndex += 1) {
                glyphs.push(this.add.text(column.x, column.y - glyphIndex * 18, "0", {
                    color: "#55ff88",
                    fontFamily: "monospace",
                    fontSize: "15px",
    ...
    showFailure() {
        const error = this.add.text(this.width / 2, this.height * 0.43, "ERROR", {
            color: "#c7ffd4",
            fontFamily: "monospace",
            fontSize: "34px",
            fontStyle: "bold",
        ...
        const failure = this.add.text(this.width / 2, this.height * 0.56, "SYSTEM FAILURE", {
            color: "#57ff85",
            fontFamily: "monospace",
            fontSize: "20px",
        ...
        this.tweens.add({ targets: error, alpha: 1, duration: 60, yoyo: true, repeat: 3 });
        this.tweens.add({ targets: failure, alpha: 1, delay: 180, duration: 70, yoyo: true, repeat: 2 });
    }
    ...
    showMainMenu() {
        ...
        const shade = this.add.rectangle(this.width / 2, this.height / 2, this.width, this.height, 0x000000, 0.52);
        const title = this.add.text(this.width / 2, 64, "N E X U S", {
            color: "#c9ffda",
            fontFamily: "monospace",
            fontSize: "30px",
            fontStyle: "bold",
        ...
        const subtitle = this.add.text(this.width / 2, 96, "SYSTEM INTERFACE", {
            color: "#54d879",
            fontFamily: "monospace",
            fontSize: "11px",
        }).setOrigin(0.5);
        ...
        menuItems.forEach((label, index) => {
            const button = this.createButton(this.width / 2, 165 + index * 54, label, 240, 38);
            this.menuLayer.add(button);
            button.on("buttondown", () => this.selectMenuOption(label));
        });

        this.menuStatus = this.add.text(this.width / 2, 392, "AGUARDANDO COMANDO...", {
            color: "#5be884",
            fontFamily: "monospace",
            fontSize: "11px",
        }).setOrigin(0.5);
        this.menuLayer.add(this.menuStatus);
        this.tweens.add({ targets: this.menuLayer, alpha: 1, duration: 420, ease: "Cubic.easeOut" });
    }
    ...
    selectMenuOption(option) {
        ...
        if (option === "SAIR") {
            ...
            window.close();
            this.time.delayedCall(250, () => this.menuStatus.setText("O NAVEGADOR BLOQUEOU O FECHAMENTO"));
            return;
        }

        this.menuStatus.setText(`${option} // DISPONÍVEL EM BREVE`);
        this.tweens.add({ targets: this.menuStatus, alpha: 0.35, duration: 120, yoyo: true, repeat: 2 });
    }

    showSoloSelection() {
        this.menuLayer.setVisible(false);
        this.soloLayer = this.add.container(0, 0).setDepth(21).setAlpha(0);
        const shade = this.add.rectangle(this.width / 2, this.height / 2, this.width, this.height, 0x000000, 0.72);
        const title = this.add.text(this.width / 2, 62, "SELECIONE SEU OPERADOR", {
            color: "#c9ffda",
            fontFamily: "monospace",
            fontSize: "20px",
            fontStyle: "bold",
        }).setOrigin(0.5);
        const portraitFrame = this.add.rectangle(this.width / 2, 205, 112, 112, 0x062b16, 0.95)
            .setStrokeStyle(2, 0x42ff84, 1)
            .setInteractive({ useHandCursor: true });
        const portrait = this.add.image(this.width / 2, 205, "start-character", 26)
            .setScale(1.45)
            .setInteractive({ useHandCursor: true });
        const name = this.add.text(this.width / 2, 285, "OPERADOR // 01", {
            color: "#9dffb9",
            fontFamily: "monospace",
            fontSize: "13px",
        }).setOrigin(0.5);
        const choose = this.createButton(this.width / 2, 350, "INICIAR SOLO", 210, 40);
        const back = this.createButton(this.width / 2, 405, "VOLTAR", 130, 30);
        back.label.setFontSize("12px");
        this.soloLayer.add([shade, title, portraitFrame, portrait, name, choose, back]);
    ...
    update(time, delta) {
        ...
        this.columns.forEach((column) => {
            ...
            if (time > column.nextChange) {
                ...
                column.glyphs.forEach((glyph, glyphIndex) => {
                    glyph.setText(this.characters[Phaser.Math.Between(0, this.characters.length - 1)]);
                    glyph.setAlpha(Math.max(0.08, column.brightness * (1 - glyphIndex / column.length)));
        ...
        if (this.mode === "intro" && elapsed > 1200 && time > this.nextMessageAt) {
            this.showSystemMessage(this.messages[this.nextMessageIndex % this.messages.length]);
        ...
        if (this.mode === "intro" && elapsed > 4450) {
            this.columns.forEach((column) => column.glyphs.forEach((glyph) => glyph.destroy()));
            this.columns = [];
            this.finishSequence();
